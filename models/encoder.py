...
class CoSTEncoder(nn.Module):
    def __init__(self, input_dims, output_dims,
        ...
        self.input_fc = nn.Linear(input_dims, output_dims)

        self.kernels = kernels

        self.tfd = nn.ModuleList(
            [nn.Conv1d(component_dims, component_dims, k, padding=k-1) for k in kernels]
        )

        self.sfd = nn.ModuleList(
            [BandedFourierLayer(component_dims, component_dims, b, 1, length=length) for b in range(1)]
        )
    ...
    def forward(self, x, tcn_output=False, mask='all_true'):  # x: B x T x input_dims
        ...
        x_s = x[:,:,:d] #season
        x_t = x[:,:,d:] #trend
        x_t = x_t.transpose(1, 2)  # B x Ch x T
        ...
        for mod in self.sfd:
            ...
            season.append(out)
        season = season[0]
        result = torch.cat([season,trend],dim=-1)
