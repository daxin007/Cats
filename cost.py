...
class PretrainDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        #print(ts.shape)
        # season sample pairs
        ts = ts[:,int(ts.shape[1]/2):]
        ts_fft = fft.rfft(ts, dim=0)
        k1 = random.randint(5,20)
        _, topk_indices = torch.topk(abs(ts_fft), k1, dim=0)
        r1 = torch.zeros_like(ts_fft)
        r1[topk_indices, torch.arange(ts_fft.shape[1])] = ts_fft[topk_indices, torch.arange(ts_fft.shape[1])]
        ts1 = fft.irfft(r1, dim=0)
        k2 = k1
        while k2 == k1:
            k2 = random.randint(5, 20)
        _, topk_indices = torch.topk(abs(ts_fft), k2, dim=0)
        r2 = torch.zeros_like(ts_fft)
        r2[topk_indices, torch.arange(ts_fft.shape[1])] = ts_fft[topk_indices, torch.arange(ts_fft.shape[1])]
        ts2 = fft.irfft(r2, dim=0)
        if ts1.shape != ts.shape:
            ts1 = torch.cat([ts1,ts1[-1:]])
            ts2 = torch.cat([ts2,ts2[-1:]])
        # trend sample pairs
        t1 = random.randint(1,5)
        pool1 = nn.AvgPool1d(kernel_size=(2*t1)+1,stride=1,padding=t1)
        #print(ts.shape)
        tt1 = pool1(ts.T).T
        t2 = t1
        while t2 == t1:
            t2 = random.randint(1,5)
        pool2 = nn.AvgPool1d(kernel_size=(2*t2)+1,stride=1,padding=t2)
        tt2 = pool2(ts.T).T
        ts1 = torch.cat((ts1,tt1),dim=1)
        ts2 = torch.cat((ts2,tt2),dim=1)
        return ts1, ts2
        # may transform ?
        #return self.transform(ts1), self.transform(ts2)
...
class CoST:
    ...
    def _eval_with_pooling(self, x, mask=None, slicing=None, encoding_window=None):
        out_t = self.net(x.to(self.device, non_blocking=True))  # l b t d
        out = out_t[:, -1]
